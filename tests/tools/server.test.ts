...
describe("HOSTAWAY_MCP_READONLY flag", () => {
  ...
  test("when HOSTAWAY_MCP_READONLY=true, only the 6 read tools are registered", async () => {
    vi.stubEnv("HOSTAWAY_MCP_READONLY", "true");

    // FakeWriteClient has updateConversation so write tools would normally be registered
    ...
    const server = createHostawayMcpServer({
      ...
      name: "hostaway-mcp-readonly-test",
      version: "0.1.0-test"
    });
    ...
    const [clientTransport, serverTransport] = InMemoryTransport.createLinkedPair();
    const readonlyClient = new Client({ name: "readonly-test-client", version: "1.0.0" });

    await Promise.all([server.connect(serverTransport), readonlyClient.connect(clientTransport)]);
  ...
  test("when HOSTAWAY_MCP_READONLY is unset, write tools are registered normally", async () => {
    ...
    const [clientTransport, serverTransport] = InMemoryTransport.createLinkedPair();
    const writesClient = new Client({ name: "writes-test-client", version: "1.0.0" });

    await Promise.all([server.connect(serverTransport), writesClient.connect(clientTransport)]);

    const result = await writesClient.listTools();
    await clientTransport.close();
